...
Write-Host "`n--- RECENT LEDGER ENTRIES ---"
$ledgerUrl = "$url/financial_ledger?user_id=eq.$profileId&order=created_at.desc&limit=20"
$ledger = Invoke-RestMethod -Uri $ledgerUrl -Headers $headers -Method Get
$ledger | Select-Object id, amount_usd, original_amount, original_currency, type, status, created_at | Format-Table
...
$subs | Select-Object id, student_id, balance_hours, status | Format-Table
