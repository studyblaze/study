const { createClient } = require('@supabase/supabase-js');
const supabase = createClient('https://exkgplqcdtoxfmxqkdpl.supabase.co', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImV4a2dwbHFjZHRveGZteHFrZHBsIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MjIxMDQ3NSwiZXhwIjoyMDg3Nzg2NDc1fQ.t7Knpc2zi0qDrjQS09AJHWRDrDrpgzFRd5rmP_KvTzE');

const LOGO_URL = 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?q=80&w=256&h=256&fit=crop';

async function run() {
    console.log('Updating Shreyash Kale profiles...');
    const { data, error } = await supabase
        .from('profiles')
        .update({ avatar_url: LOGO_URL })
        .ilike('full_name', '%Shreyash Kale%')
        .select();

    if (error) {
        console.error('Error updating profiles:', error);
        return;
    }
    
    console.log('Successfully updated profiles:', JSON.stringify(data, null, 2));
}

run();
