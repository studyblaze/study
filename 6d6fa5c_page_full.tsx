...
export default function ApplyTutorPage() {
    ...
    const validateStep = (step: Step) => {
        ...
        switch (step) {
            case 1:
                if (formData.fullName.trim().length < 3) errors.push('Full Name (min 3 characters)');
                if (!(files.profilePic !== null || profilePicPreview?.startsWith('http') || !!existingTutor?.avatar)) errors.push('Profile Photo');
                if (formData.bio.trim().length < 50) errors.push('About Me Bio (min 50 characters)');
                if (formData.birthCountry.trim() === '') errors.push('Country of Birth');
                if (formData.livingCountry.trim() === '') errors.push('Country of Residence');
                if (formData.city.trim() === '') errors.push('Current City');
                if (formData.languages.length === 0) errors.push('Languages Spoken');
                break;
            case 2:
                if (formData.subjects.length === 0) errors.push('Subjects you want to teach');
                if (formData.experience.trim().length < 50) errors.push('Teaching Experience (min 50 characters)');
                if (formData.methodology.trim().length < 50) errors.push('Teaching Methodology (min 50 characters)');
                if (formData.hourlyRate <= 0) errors.push('Hourly Rate (must be greater than 0)');
                break;
            case 3:
                if (formData.videoType === 'link') {
                    if (formData.videoLink.trim() === '') errors.push('Video Link (YouTube/Loom/Vimeo)');
                } else {
                    if (!(files.videoFile !== null || !!existingTutor?.intro_video_url)) errors.push('Intro Video File');
                }
                const hasThumb = files.videoThumbnail !== null || (thumbnailPreview && !thumbnailPreview.startsWith('blob:')) || !!existingTutor?.video_thumbnail_url || !!existingTutor?.applicationData?.videoThumbnailUrl || !!existingTutor?.applicationData?.videoThumbnail;
                if (!hasThumb) errors.push('Video Thumbnail/Cover Image');
                break;
            case 4:
                const hasCV = files.cvResume !== null || (cvPreview && !cvPreview.startsWith('blob:')) || !!existingTutor?.applicationData?.cvUrl;
                if (!hasCV) errors.push('CV or Resume');
                if (formData.fitForPlatform.trim().length < 50) errors.push('Fit Description (min 50 characters)');
                if (formData.whyHire.trim().length < 50) errors.push('Why Hire Section (min 50 characters)');
                if (formData.education.trim().length < 3) errors.push('Education Institution');
                if (formData.degreeName.trim().length < 2) errors.push('Degree Name');
                if (formData.referralSource.trim() === '') errors.push('Referral Source / How you found us');
                break;
        }
    ...
    return (
        <div style={{ 
            ...
            {/* Sara Guide Left - LARGE PHOTOREALISTIC */}
            <div className="mobile-hide" style={{
                position: 'fixed', left: -20, bottom: 0, width: 420, height: 650, zIndex: 1,
                pointerEvents: 'none', filter: 'drop-shadow(0 20px 60px rgba(0,0,0,0.5))',
                opacity: 0.95, mixBlendMode: 'screen'
            }}>
                <img src="/sarah-pro-left.png" alt="Sara Guide" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
            </div>

            {/* Sara Guide Right - LARGE PHOTOREALISTIC */}
            <div className="mobile-hide" style={{
                position: 'fixed', right: -20, bottom: 0, width: 420, height: 650, zIndex: 1,
                pointerEvents: 'none', filter: 'drop-shadow(0 20px 60px rgba(0,0,0,0.5))',
                opacity: 0.95, transform: 'scaleX(-1)', mixBlendMode: 'screen'
            }}>
                <img src="/sarah-pro-right.png" alt="Sara Support" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
            </div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
                                        ...
                                        <div>
                                            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) 280px', gap: 32 }} className="mobile-grid-1">
                                                <div>
                                                    <label style={{ display: 'block', fontSize: 13, fontWeight: 800, color: 'var(--text-muted)', marginBottom: 16, textTransform: 'uppercase' }}>Video Introduction <span style={{ color: '#ff4b82' }}>*</span></label>
                                                ...
                                                <div style={{ background: 'var(--bg)', padding: '24px', borderRadius: 24, border: '1px solid var(--border)', alignSelf: 'end' }}>
                                                    <h3 style={{ fontSize: 15, fontWeight: 800, marginBottom: 16, color: 'var(--text)' }}>Video Requirements</h3>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                                        {[
                                                            ...
                                                            <div key={t} style={{ fontSize: 13, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 10, fontWeight: 600 }}>
                                                                <CheckCircle2 size={16} color="var(--accent-green)" /> {t}
                                                            </div>
                                                        ))}
                                                    </div>
                                                    <div style={{ marginTop: 24, padding: 12, background: 'rgba(26, 115, 232, 0.05)', borderRadius: 12, fontSize: 12, color: 'var(--accent)', fontWeight: 700 }}>
                                                        Expert Tip: Smile and show your personality!
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div style={{ marginTop: 32 }}>
                                                    ...
                                                    >
                                                        <div
                                                            ...
                                                            style={{ width: 100, height: 60, borderRadius: 10, background: 'var(--bg-elevated)', border: '1px solid var(--border)', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                                        >
